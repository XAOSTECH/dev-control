...
export GIT_CONTROL_DIR  # Export to avoid SC2034 warning
...
GC_CONTAINER_CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/git-control"
GC_CONTAINER_CONFIG="$GC_CONTAINER_CONFIG_DIR/container.yaml"
GC_PROJECT_CONTAINER_CONFIG=".devcontainer.yaml"
...
show_help() {
    cat << 'EOF'
Git-Control Containerisation - Create optimised devcontainer configurations
...
CONFIGURATION:
  Global config:    ~/.config/git-control/container.yaml
  Project config:   .devcontainer.yaml (in project root)

  Configuration hierarchy (highest priority first):
    1. CLI arguments
    2. Project config (.devcontainer.yaml)
    3. Global config (~/.config/git-control/container.yaml)
...
ALIASES:
  gc-container, gc-containerise, gc-devcontainer
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        if [[ "$line" =~ ^[[:space:]]*([a-zA-Z_][a-zA-Z0-9_-]*)[[:space:]]*:[[:space:]]*(.*)[[:space:]]*$ ]]; then
            ...
            case "$key" in
                ...
                install_git_control) CFG_INSTALL_GIT_CONTROL="$value" ;;
                git_control_version) CFG_GIT_CONTROL_VERSION="$value" ;;
            esac
        fi
    done < "$file"
}
...
load_container_config() {
    ...
    CFG_INSTALL_GIT_CONTROL="${CONTAINER_DEFAULTS[install_git_control]}"
    CFG_GIT_CONTROL_VERSION="${CONTAINER_DEFAULTS[git_control_version]}"
    
    # Load global config if exists
    if [[ -f "$GC_CONTAINER_CONFIG" ]]; then
        parse_container_yaml "$GC_CONTAINER_CONFIG"
    fi
    
    # Load project config if exists
    if [[ -n "$PROJECT_PATH" && -f "$PROJECT_PATH/$GC_PROJECT_CONTAINER_CONFIG" ]]; then
        parse_container_yaml "$PROJECT_PATH/$GC_PROJECT_CONTAINER_CONFIG"
    fi
...
install-git-control: ${CFG_INSTALL_GIT_CONTROL}
git-control-version: ${CFG_GIT_CONTROL_VERSION}
EOF
...
run_interactive_config() {
    ...
    if confirm "Save these settings as global defaults?"; then
        save_container_config "$GC_CONTAINER_CONFIG"
    fi
}
...
show_activation_instructions() {
    ...
    echo -e "  Next time, run: ${GREEN}gc-container --defaults${NC}"
    echo ""
}
